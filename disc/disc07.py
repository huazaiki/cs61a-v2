...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {letter: Button(letter, lambda n: n) for letter in LOWERCASE_LETTERS}  # Try a dictionary comprehension!

    def type(self, word):
        """Press the button for each letter in word."""
        assert all([w in LOWERCASE_LETTERS for w in word]), 'word must be all lowercase'
        for letter in word:
            self.typed.append(self.keys[letter].press().letter)
...
class WinkingBear(Bear):
    ...
    """
    def __init__(self):
        self.eyes_close = False # 闭眼状态
        super().__init__()

    def next_eye(self):
        if not self.eyes_close:
