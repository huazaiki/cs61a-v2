...
def balanced(m):
    ...
    True
    """
    if is_planet(m):    #   如果 m 是 planet,直接返回 True
        return True
    else:
        """如果 m 是 mobile,那么 m 有 left arm 和 right arm
        此时计算是否平衡，左力矩 == 右力矩，还要确保如果 left arm 下面是 planet 保证其平衡
        """
        result = length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m)))
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        res = [max_path_sum(k) + label(t) for k in branches(t)]
        return max(res)
