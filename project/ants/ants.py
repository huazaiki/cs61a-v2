...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    lower_bound = None  # 攻击下限 - For LongThrower
    upper_bound = None  # 攻击上限 - For ShortThrower
    ...
    def nearest_bee(self):
        ...
        detect_place = self.place
        while detect_place and not detect_place.is_hive:
            if self.lower_bound:    # 如果有下限，则是 LongThrower
                for _ in range(self.lower_bound):
                    detect_place = detect_place.entrance
                    if not detect_place:
                        return None
            elif self.upper_bound:  # 如果有上限，则是 ShortThrower
                for _ in range(self.upper_bound):
                    if detect_place.bees:
                        return random_bee(detect_place.bees)
                    detect_place = detect_place.entrance
                    if not detect_place:
                        return None
                return None
            # 如果没有上限和下限，则为 Thrower
            if detect_place.bees:
                return random_bee(detect_place.bees)
            detect_place = detect_place.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        updated_health = self.health - amount   # 自身血量值变化

        if updated_health <= 0:
            for bees in self.place.bees[:]:
                bees.reduce_health(amount + self.damage)
        else:
            for bees in self.place.bees[:]:
