...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def select(sentence):
        changed_sentence = split(lower(remove_punctuation(sentence)))
        for word1 in changed_sentence:
            for word2 in subject:
                if word1 == word2:
                    return True
        return False
    return select
...
def furry_fixes(typed, source, limit):
    ...
    def select(word1, word2, sum):
        if sum > limit:
            ...
            if word1[0] == word2[0]:
                return select(word1[1:], word2[1:], sum)
            else:
                return select(word1[1:], word2[1:], sum + 1) + 1
