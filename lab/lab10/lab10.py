    ...
    def f(f_args, divide=None):
        if not f_args:
            ...
            if not divide:
                ...
                divide //= calc_eval(f_args.first)
            return f(f_args.rest, divide)
    return f(args)
...
def eval_and(expressions):
    ...
    def f(exp, res=True):
        if not exp:
            ...
            first_eval = calc_eval(exp.first)
            res = res and first_eval
            if res is scheme_f: # 一旦有False就立即返回 #f
                return scheme_f
            else:
                res = first_eval    # 否则我们将此次计算的结果放入res中继续向下传递
                return f(exp.rest, res)
...
def eval_define(expressions):
    ...
    #     else:
    #         name = exp.first
    #         bindings[name] = calc_eval(exp.rest)
    # return f(expressions)
